...
def replace_fields_in_document(document, first_name, last_name, username, phone_number, mod, selected_users):
    for paragraph in document.paragraphs:
        if "{F_Name}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{F_Name}", first_name)
        if "{L_Name}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{L_Name}", last_name)
        if "{Username}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{Username}", username)
        if "{Phone_Number}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{Phone_Number}", phone_number)
        if "{MOD}" in paragraph.text:
            paragraph.text = paragraph.text.replace("{MOD}", str(mod))
        ...
        for row in table.rows:
            for cell in row.cells:
                ...
                if "{Phone_Number}" in cell.text:
                    cell.text = cell.text.replace("{Phone_Number}", phone_number)
                if "{MOD}" in cell.text:
                    cell.text = cell.text.replace("{MOD}", mod)
